...
export function TaskListV2({ tasks, isStandalone = false }: Props): React.ReactNode {
  ...
  const recentCompletionStateRef = React.useRef<RecentCompletionState>({
    timestamps: new Map(),
    completedIds: new Set(tasks.filter(t => t.status === 'completed').map(t => t.id)),
  });
  const maxDisplay = rows <= 10 ? 0 : Math.min(10, Math.max(3, rows - 14));

  const now = Date.now();
  const recentCompletionTimestamps = recentCompletionStateRef.current.timestamps;
  const currentCompletedIds = new Set(tasks.filter(t => t.status === 'completed').map(t => t.id));
  for (const id of currentCompletedIds) {
    if (!recentCompletionStateRef.current.completedIds.has(id)) {
      recentCompletionTimestamps.set(id, now);
    }
  }
  for (const id of recentCompletionTimestamps.keys()) {
    if (!currentCompletedIds.has(id)) {
      recentCompletionTimestamps.delete(id);
    }
  }
  recentCompletionStateRef.current.completedIds = currentCompletedIds;

  React.useEffect(() => {
    if (recentCompletionTimestamps.size === 0) {
      return;
    }
    const currentNow = Date.now();
    let earliestExpiry = Infinity;
    for (const ts of recentCompletionTimestamps.values()) {
  ...
  }, [recentCompletionTimestamps, tasks]);
  ...
  if (needsTruncation) {
    ...
    for (const task of tasks.filter(t => t.status === 'completed')) {
      const ts = recentCompletionTimestamps.get(task.id);
    ...
    <Box flexDirection="column" width="100%">
      <Box width="100%" flexDirection="row">
        ...
        <Box flexGrow={1} flexShrink={1} minWidth={0}>
          <Text bold={isInProgress} strikethrough={isCompleted} dimColor={isCompleted || isBlocked} wrap="truncate-end">
            {task.subject}
          </Text>
        </Box>
        {showOwner && (
      ...
      {isBlocked && blockedByText && (
        <Box width="100%">
          <Text dimColor>
            {'  '}
            {figures.pointerSmall} blocked by {blockedByText}
          </Text>
        </Box>
      )}
